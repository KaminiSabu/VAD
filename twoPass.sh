...
for file in /home/swar/swar/shreeharsha/VAD/Ankita_VAD_new/audios/*.wav; do 
...
python post_proc.py "${file}_aled.txt" 10 aled ## 10 is parameter for frame size

echo "ZFFS"
python zffs.py "$file" #2< warnings.txt;
echo "ZFFS post processing"
python post_proc.py "${file}_zffs.txt" 10 zffs ## 10 is parameter for frame size
echo "ZFFS extend"
